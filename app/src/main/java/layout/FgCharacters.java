...
public class FgCharacters extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view =  inflater.inflate(R.layout.fragment_fg_characters, container, false);
        Button button = (Button) view.findViewById(R.id.search_character);
        characterWrapper = (TextInputLayout) view.findViewById(R.id.text_character);
        rv = (RecyclerView) view.findViewById(R.id.recycler_character);
    ...
    public void callService(final String character){
        ...
        Call<RetrofitMarvelResponse> request = service.getCharacterByName(character,
            ...
            public void onResponse(Response<RetrofitMarvelResponse> response) {
                ...
                if (!(respuestaMarvel.getStatus().equals(com.example.dani.marvelpedia.MainActivity.RESP_OK)) &&
                    ...
                    if (size > 0) {
                        ...
                        ResultsAPI characters;
                        //Recorreremos la lista de personajes y obtenemos imagen y su nombre para añadirlo a la lista resultados
                        for (int i = 0; i < size; i++) {
                            characters = respuestaMarvel.getData().getResults().get(i);
                            textPart = characters.getName() + salto + salto + characters.getDescription();
                            imgLink = characters.getThumbnail().getPath() + com.example.dani.marvelpedia.MainActivity.IMAGE_SIZE +
                                    characters.getThumbnail().getExtension();
                            listaR.add(new Result(characters.getId(), imgLink, textPart));
                        }
                    }
                    else{
                        Snackbar.make(rv, "Ningún personaje encontrado" ,
    ...
    public void showResults(){
        ...
        resultVA = new ResultRVAdapter(listaR, context, ResultRVAdapter.parentCh);
